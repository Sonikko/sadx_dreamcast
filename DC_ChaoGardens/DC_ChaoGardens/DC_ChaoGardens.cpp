...
extern "C"
{
	...
	__declspec(dllexport) void __cdecl Init(const char *path)
	{
		...
		EnableSSGarden = config->getBool("", "EnableStationSquareGarden", true);
		EnableMRGarden = config->getBool("", "EnableMysticRuinsGarden", true);
		EnableECGarden = config->getBool("", "EnableEggCarrierGarden", true);
		EnableLobby = config->getBool("", "EnableChaoRaceLobby", true);
		delete config;
