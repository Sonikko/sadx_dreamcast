...
void DLCObject_Display(ObjectMaster *a1)
{
	...
	if (!DroppedFrames)
	{
		...
		if (v1->CharID == 2) ProcessModelNode_AB_Wrapper(&poster1, v1->Scale.x);
		if (v1->CharID == 3) ProcessModelNode_AB_Wrapper(&poster2, v1->Scale.x);
		if (v1->CharID == 4) ProcessModelNode_AB_Wrapper(&poster3, v1->Scale.x);
		njPopMatrix(1u);
	}
}
...
void LoadStuffInTwinklePark()
{
	if (CurrentLevel == 3 && CurrentAct == 0) LoadHalloweenStuff_Act1();
	if (CurrentLevel == 3 && CurrentAct == 1) LoadHalloweenStuff_Act2();
	if (CurrentLevel == 3 && CurrentAct == 2) LoadHalloweenStuff_Act3();
	sub_4570B0();
}
...
extern "C"
{
	...
	__declspec(dllexport) void __cdecl Init(const char* path, const HelperFunctions& helperFunctions)
	{
		...
		if (ModFailsafe == false)
		{
			...
			WriteCall((void*)0x00415A6D, LoadStuffInTwinklePark);
